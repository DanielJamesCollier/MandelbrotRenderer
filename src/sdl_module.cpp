...
,   m_dpi_scaled_height{} {
    ...
    if ((m_render_texture = SDL_CreateTexture(m_renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, m_renderer_width, m_renderer_height)) == nullptr) {
        throw sdl_module_exception(std::string(SDL_GetError()));
    }
...
//-----------------------------------------------------------
int
sdl_module::get_renderer_width() const noexcept {
    return m_renderer_width;
}

//------------------------------------------------------------
int
sdl_module::get_renderer_height() const noexcept {
    return m_renderer_height; 
}

//------------------------------------------------------------
void 
sdl_module::copy_pixel_buffer_into_render_texture(std::vector<std::uint32_t> const & pixels, std::size_t row_width) {
    SDL_UpdateTexture(m_render_texture, nullptr, pixels.data(), row_width * sizeof(std::uint32_t)); 
    SDL_RenderCopy(m_renderer, m_render_texture, nullptr, nullptr); 
    SDL_RenderPresent(m_renderer);
}
